...
import { updateClientSchema } from '../../utils/schemas/client.schema';
...
const crumb = [HomeRoute, ClientsRoute, EditClientRoute];
...
const EditClient = () => {
  ...
  async function fetchActivities() {
    const resActivities = await activityService.findAll();
  ...
  async function fetchData() {
    ...
    const initialValues = {
      ...
      entrepeneur: data.entrepeneur,
      businessName: data.businessName || '',
      address: data.address || '',
      locality: data.locality || '',
      extraActivities: data.clientActivities.map((activity) => activity.id),

      clientTypes: data.clientTypes.map((typeOfClient) => typeOfClient.id),

      formOfSend: data.formOfSend,
  ...
  if (loading) return <NewClientSkeleton />;
                    ...
                    <div className='mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left'>
                      ...
                      <div className='mt-2'>
                        <p className='text-sm text-gray-500'>
                          Estás seguro que desea eliminar este cliente? Todos los
                          datos serán eliminados permanentemente. Esta acción no
                          puede ser revertida.
                        </p>
                      </div>
                    </div>
      ...
      <div className='flex flex-col bg-white rounded-lg pt-3 drop-shadow-md'>
        <Formik
          ...
          onSubmit={async (values, { setSubmitting }) => {
            ...
            console.log(values);
            try {
              const res = await clientsService.update(id, values);
              // console.log(id, values);
              toast.dismiss(loadingToast);
              toast.success(`Cliente ${res.name} actualizado correctamente`);
              setClient(values);
            } catch (error) {
              setSubmitting(false);
              toast.dismiss(loadingToast);
