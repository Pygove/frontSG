...
const DeleteProviderModal = (props) => {
  const { open, setOpen, provider, setProvider, getProviders } = props;
  const providerService = new ProvidersApi();

  const deleteProvider = async () => {
    const { id, name } = provider;
    const data = await providerService.delete(id);
    if (data) {
                  ...
                  <div className='mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left'>
                    ...
                    <div className='mt-2'>
                      <p className='text-sm text-gray-500'>
                        Estás seguro que desea eliminar este proveedor? Todos los
